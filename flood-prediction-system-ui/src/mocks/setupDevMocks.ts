...
function buildReports(): ReportsResponse {
  const flood = buildFloodPrediction()
  const today = new Date()
  const rows = Array.from({ length: 18 }, (_, i) => {
    const day = new Date(today.getTime() - (i % 7) * 24 * 60 * 60 * 1000)
    const district = flood.districts[i % flood.districts.length]!
    const rain = round(Math.max(0, district.predictedRainfallMm + ((i % 5) - 2) * 4), 1)
    return {
      id: `r_${i + 1}`,
      dateIso: day.toISOString().slice(0, 10),
      district: district.name,
      risk: riskFromRain(rain),
      predictedRainfallMm: rain,
    }
  })
  return { rows }
}
...
export function setupDevMocks() {
  ...
  const mock = new AxiosMockAdapter(api, { delayResponse: 650 })

  mock.onGet('/api/weather').reply((config) => {
    const districtName = (config.params?.district as string | undefined) ?? undefined
    return [200, buildWeather(districtName)]
  })
  mock.onGet('/api/flood-prediction').reply(() => [200, buildFloodPrediction()])
  mock.onGet('/api/reports').reply((config) => {
    const all = buildReports()
    const date = (config.params?.date as string | undefined) ?? ''
    const district = (config.params?.district as string | undefined) ?? ''
    const filtered = all.rows.filter((r) => {
      const okDate = date ? r.dateIso === date : true
      const okDistrict = district ? r.district.toLowerCase().includes(district.toLowerCase()) : true
      return okDate && okDistrict
    })
    return [200, { rows: filtered }]
  })

  mock.onPost('/api/export-powerbi').reply(() => [200, { ok: true, message: 'Queued for Power BI ingestion' }])
  mock.onPost('/api/export').reply(() => [200, { ok: true }])
  mock.onPost('/api/reports/actual-flood').reply(200, { success: true })
}
